...
fn get_disks_information(sys: &mut System, arg: String) -> Vec<String> {
    ...
    vec.push(format!("{:<20} {:<20} {:<10} {:<10} {:<10} {:<10}", "Name", "Mount Point", "Filesystem", "Total Space", "Available Space", "Used Space"));
    for disk in sys.disks() {
        vec.push(format!("{:<10} {:<10} {:<10} {:<10} {:<10} {:<10}", disk.name().to_str().unwrap(), disk.mount_point().to_str().unwrap(), str::from_utf8(disk.file_system()).unwrap(), disk.total_space()/(base.pow(power)), disk.available_space()/(base.pow(power)), disk.total_space()/(base.pow(power)) - disk.available_space()/(base.pow(power))));
    }
    return vec;
}
...
fn get_cpu_information(sys: &mut System) -> Vec<String> {
    let mut vec: Vec<String> = vec![];
    vec.push(format!("{:<10} {:<10} {:<10} {:<10}", "Brand", "Vendor ID", "Name", "Frequency"));
    for cpu in sys.cpus() {
        vec.push(format!("{:<10} {:<10} {:<10} {:<10}", cpu.brand(), cpu.vendor_id(), cpu.name(), cpu.frequency()));
    }
    return vec;
}
...
fn findbypid(pid: i32) -> Option<Process> {
